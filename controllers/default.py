...
def delete_product():
	if user_validate():
		if request.method == 'POST':
			id = request.form['id'].strip()
			if persistence.update('DELETE FROM PRODUCTS WHERE ID={}'.format(id)):
...
def update_provider():
	if user_validate():
		if request.method == 'POST':
			...
			name = request.form['name'].strip().upper()
			cnpj = request.form['cnpj'].strip()
			if persistence.update('UPDATE PROVIDERS SET NAME="{}", CNPJ="{}" WHERE ID={}'.format(name, cnpj, id)):
				flash('Fornecedor alterado com sucesso.')
				flash('success')
				log('update', 'PROVIDER: ({}, "{}", "{}") -- {} ({})'.format(id, name, cnpj, session['user'][0], datetime.now()))
			else:
...
def delete_provider():
	if user_validate():
		if request.method == 'POST':
			id = request.form['id'].strip()
			if not persistence.read_one('SELECT * FROM PRODUCTS WHERE PROVIDER={}'.format(id)) and persistence.update('DELETE FROM PROVIDERS WHERE ID={}'.format(id)):
...
def update_user():
	if user_validate():
		if request.method == 'POST':
			...
			username = request.form['username'].strip()
			password = request.form['password']
			if persistence.update('UPDATE USERS SET NAME="{}", USERNAME="{}", PASSWORD="{}" WHERE ID={}'.format(name, username, password, id)):
				flash('Usuário alterado com sucesso.')
				flash('success')
				log('update', 'USER: ({}, "{}", "{}", "{}") -- {} ({})'.format(id, name, username, password, session['user'][0], datetime.now()))
			else:
...
def delete_user():
	if user_validate():
		if request.method == 'POST':
			id = request.form['id'].strip()
			if persistence.update('DELETE FROM USERS WHERE ID={}'.format(id)):
